...
RSpec.describe Ruleur::Persistence::ActiveRecordRepository do
  ...
  describe 'with mocked ActiveRecord' do
    let(:mock_model) { double('ActiveRecordModel') }
    let(:mock_record) { double('record', payload: nil, 'payload=': nil, save!: true) }
    ...
    describe '#save' do
      it 'serializes and saves rule to model' do
        ...
        expect(mock_record).to receive(:payload=)
    ...
    describe '#all' do
      it 'loads and deserializes all rules' do
        ...
        mock_record = double('record', payload: payload)
    ...
    describe '#delete' do
      it 'deletes rules by name' do
        ...
        expect(mock_model).to receive(:where).with(name: 'test_rule').and_return(mock_relation)
        expect(mock_relation).to receive(:delete_all)
      ...
      it 'creates default AR model with serialize for old ActiveRecord' do
        ar_base = Class.new do
          class << self
            ...
            # Return false for respond_to?(:attribute_types) to trigger serialize path
